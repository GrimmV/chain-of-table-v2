...
if __name__ == "__main__":
    ...
    for idx in range(num_samples):
        ...
        with open(output_file, 'a') as file:
            ...
            file.write(f"{final_response["reasoning"]}\n")
            file.write(f"{final_response['final_response']}\n")
            # file.write(f"model evaluation: {final_response['istrue']}\n")
            
            file.write("baseline output:\n")
            file.write(f"{final_response_baseline["reasoning"]}\n")
            file.write(f"{final_response_baseline['final_response']}\n")
            # file.write(f"model evaluation: {final_response_baseline['istrue']}\n")
            
            file.write(f"label: {True if label == 1 else False}\n")
            file.write("\n")  # Add a newline for separation between entries
        
        eval_values.append({"pred": final_response["final_response"], "label": True if label == 1 else False})
        eval_values_baseline.append({"pred": final_response_baseline["final_response"], "label": True if label == 1 else False})
